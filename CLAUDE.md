...
- **`server/`** — FastAPI bridge in front of `src/`. Routers: `runs.py` (start a run + SSE stream; persists the ranked pool + applications from `on_event`), `applications.py` (CRUD + status + tags + deadline; bulk status/tag update + CSV export), `single_job.py` (URL/manual extract + generate; `start_generation()` + the worker are reused by the ranked-rescue flow), `config_api.py` (per-user yaml editor; `GET /api/secrets` for stored-key status), `master_data.py` (resume/bio/story text editors plus the structured editors behind the `/master-data` forms: `GET|PUT /api/master-data/resume/structured` writes through `src/master_resume.render_master`, `GET|PUT /api/master-data/stories/{name}/structured` through `src/story_doc.render_story` — both ruamel round-trips that preserve comments and unmodelled keys; `GET /api/master-data/story-taxonomy` serves the committed `master_data/stories/_INDEX.md` tag taxonomy grouped, so the tag picker never carries a second copy of a file that says "expand as needed"), `files.py` (`GET /api/files/{rel_path}` — the scoped replacement for the deleted `/files` mount), `tweak.py` (resume tweak via `src/tweak.apply_tweak` + inline cover-letter GET/PUT that re-renders docx/pdf), `stats.py` (aggregates), `ranked.py` (job triage: list a run's full scored pool, generate/"rescue" a non-selected job, **dismiss** a job → hidden from future runs via cross-run dedup), `ops.py` (provider connectivity test via a tiny real call + `GET /api/compare` two-run diff + structured `GET/PUT /api/llm-config` for per-workflow LLM routing), `chat.py` (**Prepwork** — Coach chat + Mock interview + Essay drafter + answer bank; see below), `studio.py` (LLM-assisted master-data authoring — generate a story / tweak story/bio/resume, preview-only), `inbox.py` + `reminders.py` (**Close the loop** — see below). Note: the provider layer exposes no token usage, so there is no $/token tracking — "insight" is run timing/throughput/score comparison only. Postgres via SQLModel (schema owned by Alembic in `server/migrations/`) holds accounts (`User`/`UserSession`/`UserSecret`), runs, applications, the ranked pool (`RankedJob`), and Prepwork (`ChatSession`/`ChatMessage`/`SavedAnswer`); each user's output folder is still source-of-truth for documents, served per-user via `GET /api/files/*`.
...
- **`web/`** — Next.js 16 (App Router, Turbopack) + Tailwind v4 + shadcn/ui + MagicUI. Nav groups: **Workspace** (`/` dashboard + upcoming-deadlines + **Reminders** card (calendar/digest), `/run` live SSE progress, `/applications` table+kanban / detail + **Sync inbox** button, `/single` 3-step wizard), **Prepwork** (`/coach` chat, `/interview` mock interview, `/essay` drafter — `/coach` + `/interview` share `components/coach/conversation-workspace.tsx` parametrized by `mode`; assistant text rendered with `react-markdown`; per-conversation job grounding via a searchable `ground-picker.tsx`; answer-bank sheet), **Insights** (`/runs` + `/runs/[id]` history/log/event-timeline + **Ranked jobs** triage tab, `/runs/compare` two-run diff, `/stats` Recharts), **Setup** (`/config` raw config.yaml editor + provider tests + stored-key status, `/workflows` visual per-workflow LLM routing editor, `/master-data` — resume tab is a section-based **Form** (identity, summaries, skills, jobs/projects/education, each an editable card with add/remove/reorder via `@dnd-kit/sortable`) backed by the structured endpoints, with the raw YAML editor surviving as an **Advanced** sub-tab; the stories tab has the same Form / Advanced split, its frontmatter edited through a **tag picker** (`components/master-data/tag-picker.tsx`) that suggests from the served taxonomy and visually marks an off-taxonomy tag, since that tag misses the +5 role-category bonus in `reference_loader._score` and the dashboard's coverage count; bio.md stays a plain text editor beside a guidance panel explaining that it is a voice sample injected into cover-letter and Prepwork prompts, never a set of fields; **"New story" from a description** and an **"Improve with AI"** panel via `components/ai-assist.tsx`; a shared `components/change-review.tsx` (fed by `lib/master-flatten.ts` for the form and `lib/resume-diff.ts` for text) shows what a save will change instead of just that it will). Application detail has a resume **version diff** (`lib/resume-diff.ts`) and inline cover-letter editing. Server state via TanStack Query, ephemeral UI via Zustand. Theme via next-themes (dark default; unified indigo accent — all chart/badge colors flow through CSS tokens in `globals.css`, light mode works). Cmd+K palette. A mounted `RunActivityWatcher` toasts when a background run finishes.
...
- **Off-peak default:** DeepSeek Flash has documented 2x peak pricing on weekdays at 01:00–04:00 and 06:00–10:00 UTC. The scripts default the daily run to the local equivalent of 20:00 GMT and warn if you pick a peak-hour time (`-FullPrice` silences the PS1 warning). The setting does nothing for Gemini, which is the default provider. Re-check DeepSeek's current pricing page before quoting a rate.
...
- `stories/*.md` — narrative stories with YAML frontmatter (`tags`, `role_fit`, `company_fit`, `one_liner`). `src/story_doc.py` owns the read/write round trip for one file; the frontmatter is edited as fields on `/master-data`, the body stays markdown. Stories can also be generated/edited by AI from that page or onboarding (see `src/content_studio.py`).
...
- **DeepSeek (default, `config.example.yaml`)**: every task inherits the global DeepSeek V4.1 Flash → Groq → Gemini → Cloudflare chain. Task blocks normally set only `thinking`, keeping provider routing in one place. Ranking, critique, extraction, and legacy relinefit disable reasoning; editorial work keeps it on. `deepseek-flash` is the canonical identifier. Its documented weekday peak pricing is 2x during 01:00–04:00 and 06:00–10:00 UTC. The content planner places stable source evidence before job-specific text so automatic prefix caching can work across a multi-job run.
- Gemini: `gemini-3.8-flash` is the default fallback model. Gemini 3 rejects sampling controls, so `src/providers/gemini_provider.py` omits them for that family.
