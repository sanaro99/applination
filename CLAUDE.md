...
data isolation, BYOK API keys) across three PRs. PR 1 (Postgres + Alembic) and
PR 2 (auth + tenant columns) are merged; PR 3 is not started. **Read
`docs/MULTI-USER-PLAN.md` before touching `server/`, `src/pipeline.py`, or
anything that reads `config.yaml` or `master_data/`** — it holds the settled
decisions, the verified facts, the traps that are easy to reintroduce, and a
record of where PR 2 diverged from the plan.

**Everything is authenticated now.** Two independent layers: a
...
otherwise. Config, master data, `/files`, and anything that spends a provider
key are `require_owner` until PR 3 makes them per-user.
