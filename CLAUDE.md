...
- **Close the loop (`server/inbox.py` + `server/reminders.py`, engine in `src/inbox.py` / `src/calendar_feed.py` / `src/digest.py`)** — post-application lifecycle. **Cross-run dedup**: each run is given an `excluded_keys` set (built in `runs.py._build_excluded_keys` from every `Application` + every dismissed `RankedJob`, keyed by `src.scrapers.dedupe_key(company,title)`); `rank_and_filter(excluded_keys=...)` and the triage pool drop those so already-applied/dismissed jobs are never re-tailored or re-surfaced. **Inbox sync** (`POST /api/inbox/sync`): `src/inbox.py` reads Gmail over **IMAP (stdlib `imaplib`, no OAuth — App Password)**, matches messages to in-flight applications by company, and `classify_email()` (LLM, global chain) labels each auto_ack/interview/rejection/offer/other → conservative **forward-only** status transitions (offer never downgraded; gated by `min_confidence`; idempotent via a processed-id set in `Setting`); sets `interview_at`/`applied_at`/`last_email_at`, always leaves a `notes` breadcrumb. **Reminders**: `GET /api/calendar.ics` serves a live iCalendar feed (deadlines + interviews) any calendar app can import; `src/digest.py` builds + SMTP-sends (stdlib `smtplib`, reuses the inbox App Password) a daily digest (deadlines, interviews, follow-up nudges for quiet apps, fresh matches). All stdlib — no Google API client dependency. Web: `components/inbox-sync.tsx` (Applications header) + `components/reminders-card.tsx` (dashboard).
...
- `inbox:` — Close-the-loop inbox sync (disabled by default). `email` + `app_password` (Gmail **App Password**, not login password), `imap_host`/`imap_port`, `scan_days`, `min_confidence` (status-change threshold), `auto_update_status`, `max_classifications` (per-sync LLM cost cap).
- `reminders:` — `digest_enabled`, `digest_to` (defaults to `user.email`), `deadline_window_days`, `follow_up_days`, `smtp_host`/`smtp_port` (digest sends via the `inbox.email`/`app_password` SMTP creds).
