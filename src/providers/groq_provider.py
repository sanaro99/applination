"""Groq provider via its OpenAI-compatible API."""
    ...
    for attempt in range(1, 4):
        try:
            ...
            if attempt < 3 and any(marker in message for marker in ("429", "503", "504", "timeout", "rate limit")):
                ...
                LOG.warning("Groq transient error (attempt %d/3), retrying in %ds: %s", attempt, delay, str(exc)[:120])
                time.sleep(delay)
                continue
            raise
...
class GroqProvider(LLMProvider):
    """GPT-OSS and other Groq-hosted models.

    Groq accepts the standard Chat Completions schema.  JSON schema support is
    attempted first, with the base JSON prompt parser as a compatibility path.
    """
    ...
    def __init__(self, api_key: str, model: str):
        ...
        self.client = OpenAI(
            api_key=resolve_api_key(api_key, "GROQ_API_KEY", provider="Groq", config_key="llm.groq.api_key"),
