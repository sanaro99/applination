...
export function MagicCard(props: MagicCardProps) {
  ...
  // Only "orb" mode needs window-level resets (the orb can linger after the
  // pointer leaves via the window edge). Gradient mode resets purely through
  // the element's own onPointerLeave, so it doesn't register global listeners —
  // important because grids render many gradient cards (e.g. the /run ranked
  // list), and 3 global listeners per card adds up fast.
  useEffect(() => {
    if (mode !== "orb") return
